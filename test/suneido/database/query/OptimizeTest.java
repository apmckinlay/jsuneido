...
public class OptimizeTest extends TestBase {
	...
	@Test
	public void table_index_selection2() {
		adm("create test (a,b,c) key(a) index(b)");
		Transaction t = db.updateTransaction();
		for (int i = 0; i < 50; ++i)
		...
		t.ck_complete();
		test1("test", "test^(a)");
		test1("test where b > ''", "test^(b) WHERE^(b)");
		test1("test where b isnt ''", "test^(b) WHERE^(b)");
	}
	...
	public void test() {
		...
		test1("hist union hist2",
			"(hist2^(id) UNION-LOOKUP^(date,item,id) hist^(date,item,id))",
	...
	private void test1(String query, String strategy1, String strategy2) {
		...
		assertTrue("got: " + actual + "\nnot: " + strategy1 + "\n or: " + strategy2,
				actual.equals(strategy1) || actual.equals(strategy2));
	}
