...
public final class GenerateGlobalReferences {
	...
	private static Ref[] makeRefs() throws Exception {
		return new Ref[] {
				...
				new Ref(Callback.class, Callback.class.getMethod("invoke",
						SuValue.class, long[].class), Callback.class.getMethod(
						"invokeVariableIndirect", SuValue.class, long[].class,
						Object[].class), Callback.class.getMethod("invoke0",
						SuValue.class), Callback.class.getMethod("invoke1",
						SuValue.class, Long.TYPE), Callback.class.getMethod(
						"invoke2", SuValue.class, Long.TYPE, Long.TYPE),
						Callback.class.getMethod("invoke3", SuValue.class,
								Long.TYPE, Long.TYPE, Long.TYPE),
						Callback.class.getMethod("invoke4", SuValue.class,
	...
	private static final class HeaderLineEditor extends LineEditor {
		...
		private void addClassMember(Class<?> clazz) {
			add("jclass", className(clazz),
					"Returns a global reference to the class "
							+ doxygenCode(clazz) + '.', doxygenCode(clazz), null);
		}
		...
		private void addConstructorMember(Constructor constructor,
			...
			add("jmethodID",
					...
					"Returns a global reference to the constructor "
							+ doxygenCode(constructor.toString()) + '.',
					doxygenCode(constructor.toString()),
		...
		private void addMethodMember(Method method) {
			CharSequence doxygenReturn = doxygenCode(method.toString());
		...
		private void addFieldMember(Field field) {
			CharSequence doxygenReturn = doxygenCode(field.toString());
			StringBuilder doxygenBrief = new StringBuilder(
					"Returns a global reference to the ");
		...
		private void addDoxygen(CharSequence doxygenBrief,
			...
			add(indent(2).append(" * Auto-generated by ")
					.append(doxygenCode(GenerateGlobalReferences.class))
					.append('.'));
			add(indent(2).append(" */"));
		}
	}
