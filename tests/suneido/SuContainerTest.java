...
public class SuContainerTest {
	...
	public void add_put() {
		...
		SuValue[] i = { new SuInteger(12), new SuInteger(34),
			new SuInteger(56), new SuInteger(78) };
		...
		c.putdata(new SuInteger(2), s[1]);
		assertEquals(3, c.size());
		assertEquals(i[0], c.getdata(SuInteger.ZERO));
		assertEquals(i[1], c.getdata(s[0]));
		assertEquals(s[1], c.getdata(new SuInteger(2)));
	...
	public void equals_hash() {
		...
		one.append(new SuInteger(123));
		assert ! one.equals(two);
		assert ! two.equals(one);
		assert one.hashCode() != two.hashCode();
		
		two.append(new SuInteger(123));
		assertEquals(one, two);
		assertEquals(two, one);
		assertEquals(one.hashCode(), two.hashCode());
		
		one.putdata(new SuString("abc"), new SuInteger(456));
		assert ! one.equals(two);
		assert ! two.equals(one);
		assert one.hashCode() != two.hashCode();
		
		two.putdata(new SuString("abc"), new SuInteger(456));
		assert one.equals(two);
		assert two.equals(one);
		assertEquals(one.hashCode(), two.hashCode());
	}
