...
public class ParseExpressionTest {
    ...
    public void test() {
		...
		code("return { x }",
			"(RETURN (BLOCK (LIST) (LIST (IDENTIFIER=x))))");
		code("return {|a,b| x }",
			"(RETURN (BLOCK (LIST (IDENTIFIER=a null) (IDENTIFIER=b null)) (LIST (IDENTIFIER=x))))");
		code("return {|@a| x }",
			"(RETURN (BLOCK (LIST (IDENTIFIER=@a null)) (LIST (IDENTIFIER=x))))");
		code("b = { x }",
